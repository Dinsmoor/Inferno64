#!/usr/bin/env bash
#
# tkafter.sh - compile and run the Tk `after' functional test headlessly.
#
...
SRC="$ROOT/tests/dis/tkafter.b"
DIS="$ROOT/dis/tests/tkafter.dis"
DISP=":${DISPLAY_NUM:-91}"
...
Xvfb "$DISP" -screen 0 640x480x24 >/tmp/xvfb_tkafter.log 2>&1 &
XPID=$!
trap 'kill $XPID 2>/dev/null' EXIT
sleep 1

OUT=$(DISPLAY=$DISP EMUCRASH=1 timeout 40 "$EMU" -r"$ROOT" -g320x240 /dis/tests/tkafter.dis 2>&1)
echo "$OUT"
