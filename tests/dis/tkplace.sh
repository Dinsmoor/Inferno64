#!/usr/bin/env bash
#
# tkplace.sh - compile and run the Tk `after' functional test headlessly.
#
...
SRC="$ROOT/tests/dis/tkplace.b"
DIS="$ROOT/dis/tests/tkplace.dis"
DISP=":${DISPLAY_NUM:-91}"
...
Xvfb "$DISP" -screen 0 640x480x24 >/tmp/xvfb_tkplace.log 2>&1 &
XPID=$!
trap 'kill $XPID 2>/dev/null' EXIT
sleep 1

OUT=$(DISPLAY=$DISP EMUCRASH=1 timeout 40 "$EMU" -r"$ROOT" -g320x240 /dis/tests/tkplace.dis 2>&1)
echo "$OUT"
