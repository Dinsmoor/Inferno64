...
- **docs/ON_DIS.md** — Dis virtual machine, bytecode format, GC, channels
- **docs/ON_LIMBO.md** — Limbo language, compiler, module system
- **docs/ON_KERNEL.md** — Kernel internals, 9P protocol, device drivers, process management
- **docs/ON_GRAPHICS.md** — Draw, Tk, prefab, wm windows (incl. software-rendered frames)
- **docs/ON_3D.md** — raylib-in-Limbo: Raymath, the `$Raster3` C kernel, OBJ loader
- **docs/ON_DLM.md** — Dynamically Loaded Modules (libdynld/dynld): what runtime-loadable native C modules are, why they're stubbed on hosted emu *by design*, no 64-bit backend, and why DLM ≠ loading Linux drivers
...
silently. Full details in [`docs/ON_C_IN_DIS.md`](docs/ON_C_IN_DIS.md).
...
| How is .dis file structured? | `libinterp/load.c` + `docs/ON_DIS.md` |
...
| How does Limbo → Dis compilation work? | `limbo/` + `docs/ON_LIMBO.md` |
