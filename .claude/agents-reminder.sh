...
[ -f AGENTS_INPRO.md ] || exit 0
echo "Durable project knowledge lives in AGENTS_*.md. Before deep debugging/building/graphics/etc., consult the relevant file (start: AGENTS_INPRO.md). Do NOT read them all every turn — just the one that fits the task. Topic map:"
for f in AGENTS_*.md; do
	[ -f "$f" ] || continue
