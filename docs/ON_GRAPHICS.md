...
## Theming (cursors, colours, fonts, window chrome)

Runtime theming — the Tk palette and default font, the window titlebar, and the
mouse cursor (mono, colour, animated `.cur`/`.ani`) — has its own reference:
**[`ON_THEMING.md`](ON_THEMING.md)**.  `wm/theme` is the front end; `wm/wm`
relays changes live and persists the login default.
...
| `appl/wm/theme.b` | `wm/theme` — cursor + colour/font theming (see ON_THEMING.md) |
