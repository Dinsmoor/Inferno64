/*
 * devtls - modern TLS (1.2/1.3) layered onto a connection, backed by mbedTLS.
 *
 * Usage (mirrors devssl's "push onto an fd" model):
 ...
 *	read/write(data, ...)			# cleartext; mbedTLS does the crypto
 *	read("#T/<conv>/status")		# "version cipher verify=<flags>\n"
 *
 * The TLS bytes flow over the underlying Chan via the normal device layer
 * (devtab bread/bwrite), so blocking is handled by the emu I/O framework just
 * like devssl -- no manual release()/acquire() needed (mbedTLS crypto is CPU
 * only).  BIO callbacks wrap the underlying I/O in waserror() so an Inferno
 * error() longjmp is converted to an mbedTLS error return instead of unwinding
 * through mbedTLS's C stack.
 */
#include	"dat.h"
#include	"fns.h"
#include	"error.h"
...
static char Defcafile[] = "/etc/ssl/certs/ca-certificates.crt";
...
dohandshake(Tstate *s)
{
	...
	if(s->verify != MBEDTLS_SSL_VERIFY_NONE){
		...
		ret = mbedtls_x509_crt_parse_file(&s->cacert, cafile);
		if(ret < 0)
