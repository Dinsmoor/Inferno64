/*
 * Realtek RTL8110S/8169S and RTL8168/8111 Gigabit Ethernet Controllers.
 *
 * Ported from 9front's ether8169.c to the native aarch64 kernel over the
 * os/drivers PCIe seam (pci.h).  qemu -M virt does NOT emulate this part, so
 * this driver is build-tested only here; it targets real PCIe Realtek NICs
 * (common on arm64 boards).  Seam adaptations vs the x86 original:
 *   - the chip is reached through its BAR0 (an I/O BAR), which native pci.c
 *     maps into the PCIe host bridge's I/O window as a CPU address, so the
 *     csr accessors are plain MMIO derefs, not x86 in/out, and there is no
 *     ioalloc/iofree.
 *   - DMA is to physical addresses (PCIWADDR == PADDR), 64-bit-capable via
 *     the descriptors' addrlo/addrhi pair; qemu/most arm PCIe is coherent.
 *   - the legacy INTx line is delivered as a GIC SPI via p->intl and wired
 *     by devether (edev->irq/edev->interrupt), so the driver's own
 *     intrenable()/intrdisable() calls are dropped.
 *   - pcienable()/pcidisable() (clock/power gating) maps to pcisetbme().
 * Kernel APIs this port relies on (all already landed for the Intel NICs):
 * the Bpool buffer pool (iallocbp/growbp), the MiiPhy*-based ethermii, and
 * Maxcard-decoupled devether.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
#define PCIWADDR(x)	((uvlong)PADDR(x))	/* 64-bit DMA via addrlo/addrhi */
#define NEXT(x, n)	(((x)+1)%(n))		/* was os/pc/io.h */
#define ROUNDUP(s, sz)	ROUND((s), (sz))	/* was os/pc/io.h */
#define mallocalign(n, a, o, sp)	xspanalloc((n), (a), (sp))

/*
 * 9front carries ethersetspeed()/ethersetlink() in devether to resize the
 * output queue and gate the link; Inferno's simpler Netif has none of that
 * machinery (f/bypass/mactab/qsetlimit), so here they just record the
 * speed/link the way the native Netif consumes them.
 */
static void
ethersetspeed(Ether *e, int mbps)
{
	e->mbps = mbps;
}

static void
ethersetlink(Ether *e, int link)
{
	e->link = link;
}
...
typedef struct Ctlr {
	...
	uintptr	port;			/* MMIO base (BAR0, mapped I/O window) */
	Pcidev*	pcidev;
	Ctlr*	next;
	int	active;
...
#define csr8r(c, r)	(*(volatile uchar*) ((c)->port+(r)))
#define csr16r(c, r)	(*(volatile ushort*)((c)->port+(r)))
#define csr32r(c, r)	(*(volatile u32int*)((c)->port+(r)))
#define csr8w(c, r, b)	(*(volatile uchar*) ((c)->port+(r)) = (uchar)(b))
#define csr16w(c, r, w)	(*(volatile ushort*)((c)->port+(r)) = (ushort)(w))
#define csr32w(c, r, l)	(*(volatile u32int*)((c)->port+(r)) = (u32int)(l))
...
static long
rtl8169ifstat(Ether *edev, void *a, long nn, ulong offset)
{
	u64int pa;
	Ctlr *ctlr;
	Dtcc *dtcc;
...
rtl8169attach(Ether* edev)
{
	...
	/* devether wires the GIC SPI from edev->irq/edev->interrupt */
	if(waserror()){
		rtl8169halt(ctlr);
...
rtl8169pci(void)
{
	...
	while(p = pcimatch(p, 0, 0)){
		...
		/*
		 * BAR0 is the chip's I/O BAR; native pci.c has mapped it into
		 * the PCIe host bridge's I/O window, so its mem[].bar is a CPU
		 * address we reach with plain MMIO (no ioalloc).
		 */
		if(p->mem[0].size == 0 || (p->mem[0].bar & 1) == 0)
			continue;
		port = (uintptr)(p->mem[0].bar & ~(uvlong)0x3);
		...
		if(vetmacv(ctlr, &macv) == -1){
			print("rtl8169: %.4ux/%.4ux: unknown mac %.8ux\n",
				p->vid, p->did, macv);
			free(ctlr);
			continue;
		}
...
rtl8169pnp(Ether* edev)
{
	...
	/*
	 * Claim the first unused controller (the native seam has no
	 * plan9.ini port= override to match against).
	 */
