 ...
 * may need work.
 *
 * Ported from os/pc to the native aarch64 kernel over the os/drivers PCIe
 * seam (pci.h).  The chip's registers are reached through its I/O BAR,
 * which on arm64 is a memory-mapped CPU address (the PCIe host bridge's
 * I/O window), so the csr accessors are plain MMIO, not x86 in/out.  PCI
 * DMA is to physical addresses (PCIWADDR == PADDR), and qemu's PCIe DMA
 * is cache-coherent so the ring buffers need no explicit flush.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
typedef struct Ctlr {
	uintptr	port;			/* MMIO base (chip's I/O BAR, memory-mapped) */
	Pcidev*	pcidev;
	Ctlr*	next;
	int	active;
	int	id;
...
rtl8139match(Ether* edev, int id)
{
	...
	for(ctlr = ctlrhead; ctlr != nil; ctlr = ctlr->next){
		...
		/*
		 * The host bridge already placed and enabled the BARs
		 * (os/drivers/pci.c).  BAR0 is the chip's I/O register block;
		 * on arm64 it is the memory-mapped CPU address of the PCIe
		 * I/O window, used directly as the csr base.
		 */
		ctlr->port = (uintptr)(p->mem[0].bar & ~(uvlong)0x3);
...
rtl8139pnp(Ether* edev)
{
	...
	/*
	 * Search the found controllers for one matching any of the known
	 * vid+did pairs.  (The x86 plan9.ini "id=" override is gone; reset()
	 * is called once per instance via edev->ctlrno.)
	 */
	USED(id);
	ctlr = nil;
