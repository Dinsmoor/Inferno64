/*
 * NVM Express, ported from 9front (port/sdnvme.c) to the native aarch64
 * kernel over the os/drivers PCIe seam (pci.h).  The controller has a
 * single memory BAR (identity-mapped on qemu virt, so vmap == address),
 * and all queues/PRP lists are page-aligned DMA to physical addresses
 * (PCIWADDR == PADDR).  devsd here is the older Inferno variant: real I/O
 * goes through the bio() entry (nvmebio drops straight in), rio() is a
 * stub, and 9front's sdfakescsi layer is not used.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
#define vmap(pa, n)	KADDR(pa)			/* identity-mapped MMIO */
#define mallocalign(n, a, s, sp)	xspanalloc((n), (a), (s))
#define dmaflush(rw, a, n)	coherence()		/* qemu DMA is coherent */
#define vunmap(a, n)		USED(a)			/* identity-mapped, nothing to undo */
#define pcienable(p)		(0)			/* enumerator already enabled decode */
#define pcidisable(p)		USED(p)
#define sdaddfile(u,nm,pm,usr,rd,wr)	USED(rd)	/* no sdaddfile in native devsd */
...
static long
nvmebio(SDunit *u, int lun, int write, void *a, long count, long lba)	/* native bio: long lba */
{
	u32int nsid, s, n, m, *e;
	Ctlr *ctlr;
	uchar *p;
...
{
	/*
	 * The native (older Inferno) devsd has no sdfakescsi CDB layer; raw
	 * SCSI is unsupported.  Block I/O reaches the controller through the
	 * bio() entry (nvmebio) instead, exactly as sd-virtio does.
	 */
	USED(r);
	return SDeio;
}
...
readsmart(SDunit *u, Chan *, void *a, long n, vlong off)
{
	...
	info = malloc(0x1000);	/* freeable (free()d via waserror); not mallocalign */
	if(info == nil)
		error(Enomem);
	if(waserror()){
		free(info);
...
nvmeonline(SDunit *u)
{
	...
	ctlr = u->dev->ctlr;
	/* freeable buffer: malloc (16-aligned); qcmd's PRP1+PRP2 covers the
	 * 2-page span.  NOT xspanalloc (mallocalign) — this is free()d below. */
	if((info = malloc(0x1000)) == nil)
		return 0;
...
/*
 * Native devsd names each unit "<sdev->name><subno>", and the id() ifc is
 * where that name is set (cf. sd-virtio's vblkid).  Without it the unit
 * appears as "<nil>0".
 */
static SDev*
nvmeid(SDev *sdev)
...
/* native (older Inferno) SDifc field order: legacy/id after pnp, stat at tail */
SDifc sdnvmeifc = {
	"nvme",				/* name */
	...
	nvmeonline,			/* online */
	nvmerio,			/* rio */
	nil,				/* rctl (9front nvmerctl is rtopctl-shaped) */
	nil,				/* wctl */
