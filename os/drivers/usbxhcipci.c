...
/*
 * PCIe seam shims (identical to the other os/drivers PCI drivers):
 * memory BARs are identity-mapped on qemu virt, so vmap is just KADDR and
 * PCI DMA addresses equal physical addresses.  The bus-master enable and
 * decode were already turned on by the enumerator (pci.c), so pcienable /
 * pcidisable are no-ops here.
 */
#define PCIWADDR(va)		PADDR(va)
#define vmap(pa, n)		KADDR(pa)
#define vunmap(a, n)		USED(a)
#define pcienable(p)		(0)
#define pcidisable(p)		USED(p)

#ifndef Pcibcserial
#define Pcibcserial	0x0c		/* base class: serial bus controller */
#define Pciscusb	0x03		/* sub class: USB */
#endif
