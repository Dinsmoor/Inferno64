 ...
 * seam (pci.h).  BAR0 is the CSR block (BAR3 the MSI-X table); on arm64 the
 * PCIe MMIO window is identity-mapped (KADDR(p)==p — os/drivers/pci.c), so the
 * 9front vmap()/vunmap() calls become direct use of the BAR address.  9front's
 * pcienable() (clock/power gating, absent here) maps to pcisetbme(); the legacy
 * INTx line is delivered as a GIC SPI via p->intl and wired by devether
 * (edev->irq), so the driver's own intrenable() call is dropped.  All deps were
 * already landed for the 82563/8169 ports — this is pure per-file seam work.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
/*
 * This driver only ever programs 32-bit DMA addresses: the high descriptor-
 * base dwords (Rbah/Tdbah) and the descriptor addr[1] words are hard-wired to
 * 0 below, so PCIWADDR is the 32-bit form — faithful to upstream and to the
 * igbe/rtl8139 32-bit-DMA parts.  (Contrast 82563/8169, which write the high
 * dwords and therefore need (uvlong)PADDR.)
 */
...
scan(void)
{
	...
	while(p = pcimatch(p, 0x8086, 0)){
		...
		/* BARs are identity-mapped in the PCIe window — no vmap */
		io = p->mem[pciregs].bar & ~0xf;
		mem = (void*)(uintptr)io;
		iomsi = p->mem[pcimsix].bar & ~0xf;
		memmsi = (void*)(uintptr)iomsi;
		c->p = p;
		c->io = io;
		c->reg = (u32int*)mem;
