/*
 * Generic MII/PHY management (9front's MiiPhy*-based ethermii, with clause-45
 * MMD support), vendored from os/port into the native os/drivers pool so PHY-
 * driving PCI NICs (igbe, 82563, ...) can link it.  Portable bit-twiddling
 * over a driver-supplied mir/miw pair; dropped the os/pc "io.h" include and
 * pointed etherif.h at the pool.  addmiibus/delmiibus are no-op hooks unless
 * the (unported) #mii debug device overrides them.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
