/*
 * Generic SCSI command helpers for devsd (scsiverify/scsionline/scsibio):
 * translate block I/O into SCSI CDBs and issue them through a driver's
 * rio().  From the in-tree boot-loader sdscsi (matches the native devsd
 * vintage).  Used by sd-ahci; sd-virtio/sd-nvme implement bio directly.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
/* boot sdmalloc(p,sz): reuse p if non-nil (zeroed), else allocate */
#undef sdmalloc
static void*
...
extern int return0(void*);	/* native proc.c */
