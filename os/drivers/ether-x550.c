 ...
 * seam (pci.h).  Structurally a near-clone of the 82598 driver: BAR0 is the
 * CSR block (BAR4 the MSI-X table), both identity-mapped in the PCIe MMIO
 ...
 * All deps were already landed for the 82563/8169/82598 ports.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
/*
 * This driver only programs 32-bit DMA addresses: the high descriptor-base
 * dwords (Rdbah/Tdbah) and descriptor addr[1] are hard-wired to 0, so PCIWADDR
 * is the 32-bit form (faithful to upstream; same as igbe/rtl8139/82598).
 */
...
multicast(void *a, uchar *ea, int on)
{
	 ...
	 * so it's never safe to clear a filter bit.  see the long note
	 * in the 9front source: Mcstctrl.Mo==0 uses addr bits [47:36].
	 */
	i = ea[5] >> 1;			/* register index = 47:41 (7 bits) */
	b = (ea[5]&1)<<4 | ea[4]>>4;	/* which bit = 40:36 (5 bits) */
	b = 1 << b;
	if(on)
		c->mta[i] |= b;
//	else
