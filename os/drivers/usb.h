...
{
	/*
	 * 9front embeds ISAConf here for hardware info; the native kernel's
	 * ISAConf.type is a char[] array, which collides with this code's
	 * `hp->type = "xhci"` pointer assignment.  We only need three of
	 * those fields, so spell them out instead of embedding.
	 */
	char*	type;				/* controller type name ("xhci") */
	uintptr	port;				/* mmio base (matched against ctlr->base) */
	int	irq;				/* interrupt (GIC SPI on this port) */
