...
/*
 * Native-kernel shims for the 9front USB framework (see ON_PORTING.md
 * Part III).  This is the host-controller-independent #u device; the
 * controller plugs in via Hciimpl (usbxhci.c) and registers itself with
 * addhcitype() from its link function (usbxhcipci.c).
 *
 *   eqlock     - 9front's error-unwinding qlock; plain qlock here.
 *   isaconfig  - ISA/plan9.ini probing; there is no ISA bus, so report
 *                "no device" and let hciprobe() fall through to the PCI
 *                controller types registered via addhcitype().
 *   BUSUNKNOWN - from io.h (not included); the framework only stores it.
 *   Etoolong   - not in the native error.h.
 */
#define eqlock(q)		qlock(q)
#define isaconfig(t, n, i)	(0)
#define BUSUNKNOWN		(-1)
extern char* getconf(char*);		/* os/aarch64/stubs.c (returns char*, not int) */
static char Etoolong[] = "name too long";
