 ...
 * seam (pci.h).  BAR0 is the CSR block; on arm64 the PCIe MMIO window is
 * identity-mapped (KADDR(p)==p — os/drivers/pci.c), so the 9front vmap()/
 * vunmap() calls become direct use of the BAR address.  9front's pcienable()
 * (clock/power gating, absent here) maps to pcisetbme(); the legacy INTx line
 * is delivered as a GIC SPI via p->intl and wired by devether (edev->irq),
 * so the driver's own intrenable() call is dropped.  The receive ring uses the
 * Bpool buffer-pool API now ported into os/port/allocb.c.  DMA == PADDR.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
{	
	print("#l%d: %s: %s: %s\n", ctlr->edev->ctlrno, cname(ctlr), up->text, up->env->errstr);
...
fload(Ctlr *c)
{
	...
	/* flash BAR, identity-mapped like the CSR block — no vmap */
	va = (void*)(uintptr)(c->pcidev->mem[1].bar & ~(uvlong)0xF);
	if(va == nil)
		return -1;
	f.reg = va;
	f.reg32 = va;
...
setup(Ctlr *ctlr)
{
	...
	p = ctlr->pcidev;
	/* BAR0 is the CSR block, identity-mapped in the PCIe window — no vmap */
	ctlr->nic = (void*)(uintptr)ctlr->port;
	if(ctlr->nic == nil){
		print("%s: can't map %llux\n", cname(ctlr), (uvlong)ctlr->port);
		return -1;
	}
	if(i82563reset(ctlr))
		return -1;
...
pnp(Ether *edev, int type)
{
	...
	/*
	 * Claim the next unclaimed controller (of the requested type, if any).
	 * The native Ether has no port/tbdf, so binding is first-come.
	 */
	...
	/*
	 * Linkage to the generic ethernet driver.  devether wires the interrupt
	 * from edev->irq/edev->interrupt (legacy INTx as a GIC SPI).
	 */
