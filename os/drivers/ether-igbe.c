 ...
 * seam (pci.h).  BAR0 is the chip's 32KB CSR block; on arm64 the PCIe MMIO
 * window is identity-mapped (KADDR(p)==p — see os/drivers/pci.c), so the BAR
 * physical address is used directly as the csr base and the os/pc vmap()/
 * vunmap() calls are gone.  DMA is to physical addresses (PCIWADDR==PADDR);
 * qemu's PCIe DMA is cache-coherent, the rings carry an explicit coherence().
 * Interrupt is the legacy INTx line delivered as a GIC SPI via p->intl, wired
 * by devether.  The generic PHY layer comes from the vendored ether-mii.c.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
typedef struct Ctlr {
	uintptr	port;			/* CSR BAR phys==virt (identity-mapped) */
	Pcidev*	pcidev;
	Ctlr*	next;
	Ether*	edev;
	int	active;
...
igbepci(void)
{
	...
	while(p = pcimatch(p, 0, 0)){
		...
		/*
		 * BAR0 is the CSR block, already placed/enabled by pci.c and
		 * identity-mapped in the PCIe MMIO window, so the physical
		 * address is the CPU address — no vmap.
		 */
		mem = (void*)(uintptr)(p->mem[0].bar & ~(uvlong)0x0F);
		if(mem == nil){
		...
		switch(cls){
			...
			case 0xFF:
				/*
				 * qemu (and some BIOSes) leave the cache-line-size
				 * register uninitialised; default it to 32 bytes
				 * and write it back rather than refuse the card.
				 * (9front e3cf65067 / 35d8ccee1.)
				 */
				cls = 0x08;
		...
		ctlr->port = p->mem[0].bar & ~(uvlong)0x0F;
		ctlr->pcidev = p;
...
igbepnp(Ether* edev)
{
	...
	/*
	 * Claim the next unclaimed controller in scan order.  (The os/pc
	 * plan9.ini "port=" override is gone; the native Ether has no port/
	 * tbdf fields, so binding is purely first-come per edev->ctlrno.)
	 */
