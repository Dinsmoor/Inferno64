...
/*
 * AHCI SATA, ported to the native aarch64 kernel over the os/drivers PCIe
 * seam (pci.h).  Based on the in-tree boot-loader sdiahci (Inferno
 * dialect, self-contained ahci.h, polled with delay()), not 9front's
 * enclosure-LED-entangled version.  The AHCI ABAR is BAR5, a memory BAR
 * identity-mapped on qemu virt, so the boot loader's upamalloc/KADDR
 * mapping is a no-op here; DMA (command list/table/FIS) is to physical
 * addresses (PCIWADDR == PADDR), and the interrupt is a GIC SPI.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
#define PCIWADDR(x)	PADDR(x)			/* PCI DMA addr == phys */
#define upamalloc(pa, n, al)	(pa)			/* BAR already in device map */
#define getconf(s)	(nil)				/* no plan9.ini */
...
struct Ctlr {
	...
	uchar	*mmio;
	u32int	*lmmio;		/* 32-bit HBA regs (NOT ulong: 8 bytes on LP64) */
	Ahba	*hba;
...
aesleep(Aportm *, Asleep *a, int ms)
{
	...
	/*
	 * Bring-up is polled; m->ticks does not advance reliably in this
	 * context, so bound the spin with delay() (1ms units) instead.
	 */
	for(i = 0; i < ms && (a->p->ci & a->i) != 0; i++)
		delay(1);
}
...
ahciconfigdrive(Ahba *h, Aportc *c, int mode)
{
	...
	/*
	 * Bring-up here is fully polled: checkdrive()/ahciwait() spin on
	 * sstatus/ci/task, so port interrupts are not required.  Leaving
	 * PxIE unmasked makes the post-reset DHRS (and any device-fault D2H)
	 * re-assert forever as a GIC SPI storm.  Keep PxIE masked.
	 */
	p->ie = 0;
...
reset:
		...
		case 0x103:
			...
			s = d->port->task&0xff;
			/*
			 * Real hardware presents DRDY (1<<6) in the post-reset
			 * D2H FIS; qemu deliberately posts 0x30 (WRERR|SEEK, no
			 * DRDY) for a hard disk until the first command runs.
			 * So proceed to identify once the link is up and the
			 * drive is neither busy nor requesting data, rather than
			 * waiting for a DRDY that never comes under qemu.
			 */
			if(s == 0x7f || (s & (ASbsy|ASdrq)))
				break;
			iunlock(d);
			newdrive(d);
			ilock(d);
...
iaenable(SDev *s)
{
	...
	if(!c->enabled) {
		...
		pcisetbme(c->pci);
		intrenable(c->pci->intl, iainterrupt, c, BusCPU, "ahci");
...
iasetupahci(Ctlr *c)
{
	...
	/*
	 * Real ICH needs the AHCI MAP register (cfg 0x90) to leave legacy IDE
	 * mode and the PI register forced.  qemu's ich9-ahci is AHCI-only and
	 * already reports PI; poking 0x90 there knocks it out of AHCI mode (PI
	 * reads 0 -> no ports).  Skip both; the ghc AE bit above is enough.
	 * TODO: gate behind real-ICH detection for the future x86_64 target.
	 */
	if(0){
		c->lmmio[0xc/4] = (1<<6) - 1;		/* force PI = ports 0-5 */
		pcicfgw8(c->pci, 0x90, 1<<6 | 1<<5);	/* ich9: select AHCI mode */
	}
}
...
loop:
	while((p = pcimatch(p, 0, 0)) != nil){
		...
		pcisetbme(p);		/* configdrive() below DMAs; need bus-master on now, not at iaenable() */
...
iaid(SDev* sdev)
{
	...
	for(; sdev; sdev = sdev->next){
		...
		for(i = 0; i < NCtlr; i++)
			if(c == iactlr + i)
				sdev->idno = 'E' + i;
		snprint(buf, sizeof(buf), "sd%c", sdev->idno);
		kstrdup(&sdev->name, buf);
	}
	return nil;
}
