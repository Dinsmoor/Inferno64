 ...
 *
 * Ported from 9front to the native aarch64 kernel over the os/drivers PCIe
 * seam (pci.h).  BAR0 is the CSR block, identity-mapped in the PCIe MMIO
 * window (no vmap).  9front's pcienable() (clock/power gating, absent here)
 * maps to pcisetbme(); legacy INTx is delivered as a GIC SPI via p->intl and
 * wired by devether (edev->irq), so the driver's own intrenable() is dropped.
 * The PHY is driven through the MiiPhy* (clause-45-capable) ethermii landed
 * for the 82563 port; the rx ring uses the Bpool buffer-pool API in allocb.c.
 * dmaflush() is a no-op here: the os/drivers seam assumes cache-coherent DMA,
 * so the coherence() barrier the caller already issues is sufficient.
 */
...
/* 64-bit DMA: descriptors carry a full 64-bit address (Rtxraddrhi/addr>>32) */
#define PCIWADDR(x)	((uvlong)PADDR(x))
#define mallocalign(n, a, o, sp)	xspanalloc((n), (a), (sp))

/*
 * coherent-DMA seam: the device snoops the CPU cache, so descriptor writes
 * need only the coherence() barrier the caller already issues — no explicit
 * cache clean.  (A non-coherent SoC would implement real cache maintenance.)
 */
static void
dmaflush(int, void *, int)
{
}

/*
 * 9front's netif link/speed setters aren't in Inferno's netif; the Netif
 * embedded in Ether carries mbps and link directly, so map onto those.
 */
static void
...
struct Ctlr {
	Lock;			/* interrupt-mask lock (ilock(c)) */
	QLock;
	/*
	 * named (not anonymous) so its embedded Lock is not promoted into
	 * Ctlr's scope, where it would collide with the explicit Lock above:
	 * Inferno's Rendez embeds an anonymous Lock, and gcc -fplan9-extensions
	 * (unlike kencc) rejects two anonymous Locks at the same level.  This
	 * is the link-status rendez: sleep(&c->rendez)/wakeup(&c->rendez).
	 */
	Rendez   rendez;
...
i225proclink(void *a)
{
	...
	while (waserror())
		;
	/* process link status */
	for (;; sleep(&c->rendez, return0, nil)) {
		phy = c->mii->curphy;
		if (phy == nil) {
			/* phy missing? */
			continue;
...
i225attach(Ether *e)
{
	...
	snprint(n, sizeof(n), "#l%d", e->ctlrno); kproc(n, i225proclink, c, 0);
	snprint(n, sizeof(n), "#l%dt", e->ctlrno); kproc(n, i225proctx, c, 0);
	snprint(n, sizeof(n), "#l%dr", e->ctlrno); kproc(n, i225procrx, c, 0);
	snprint(n, sizeof(n), "#l%ds", e->ctlrno); kproc(n, i225procstat, c, 0);
...
static long
i225ifstat(Ether *e, void *a, long n, ulong offset)
{
	Ctlr *c;
	char *buf, *p, *q;
	int i;
...
i225pci(void)
{
	...
	for (p = nil; p = pcimatch(p, 0x8086, 0);) {
		...
		/* BAR0 is identity-mapped in the PCIe window — no vmap */
		c->portmmio = (u32int*)(uintptr)c->port;
...
loop:
	...
	/*
	 * legacy INTx is delivered as a GIC SPI; devether wires the handler
	 * from edev->irq/edev->interrupt, so we do NOT call intrenable here.
	 */
	e->interrupt = i225intr;
