/*
 * Intel High Definition Audio (HDA / Azalia) controller, ported from 9front
 * (pc/audiohda.c) onto the os/drivers PCIe seam (pci.h).  Registers an Audio
 * card with #A (../port/devaudio.c) via audiohdalink()->addaudiocard.  The
 * single memory BAR is identity-mapped on qemu virt (vmap == KADDR) and the
 * CORB/RIRB rings + stream buffer-descriptor lists are page-aligned DMA to
 * physical addresses (PCIWADDR == PADDR); completion is INTx on the GIC.
 * Verified against qemu -device intel-hda -device hda-output.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
hdareset1(Audio *adev, Ctlr *ctlr)
{
	...
	intrenable(irq, hdainterrupt, adev, BusCPU, "hda");
