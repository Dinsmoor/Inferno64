...
/*
 * Native-kernel shims for the 9front xHCI controller driver.
 *
 *   dmaflush  - qemu's emulated DMA is cache-coherent with the CPU, so a
 *               plain barrier suffices (same as sd-nvme).
 *   mallocalign/freealign - the native pool has no aligned allocator, and
 *               this driver free()s its rings/contexts, so xspanalloc (no
 *               free) won't do.  We over-allocate, align, and stash the
 *               original malloc pointer just below the returned block.
 *               NOTE: the "span" (no-boundary-cross) argument is ignored;
 *               harmless under qemu's non-boundary-checked DMA.
 *
 * PADDR / KZERO / getconf / coherence / intrenable / kproc / pexit /
 * allocb are the real native primitives.
 */
#define dmaflush(rw, a, n)	coherence()

extern char* getconf(char*);		/* os/aarch64/stubs.c (returns char*, not int) */

static void*
mallocalign(ulong size, int align, ulong span, ulong span2)
{
	uchar *p, *a;

	USED(span); USED(span2);
	if(align < (int)sizeof(void*))
		align = sizeof(void*);
	p = malloc(size + align + sizeof(void*));
	if(p == nil)
		return nil;
	a = (uchar*)(((uintptr)(p + sizeof(void*) + align - 1)) & ~((uintptr)align - 1));
	((void**)a)[-1] = p;
	memset(a, 0, size);
	return a;
}
...
struct Port
{
	...
	volatile u32int	*reg;	/* MMIO port register set */
};
...
struct Ctlr
{
	...
	volatile u32int	*opr;	/* operational registers (MMIO; volatile) */
	volatile u32int	*rts;	/* runtime registers (MMIO; volatile) */
	volatile u32int	*dba;	/* doorbell array (MMIO; volatile) */
...
mframe(Ctlr *ctlr)
{
	u32int mu;
	do {
		mu = (ctlr->rts[MFINDEX] & (1<<14)-1) |
			(ctlr->mframe & ~((1<<14)-1));
	} while((int)(mu - ctlr->mframe) < 0);
	return mu;
}
...
static volatile u32int*
xecp(Ctlr *ctlr, uchar id, volatile u32int *p)
{
	u32int x;
	volatile u32int *e;
...
release(Ctlr *ctlr)
{
	...
	for(i=0; i<nelem(ctlr->er); i++){
		freering(&ctlr->er[i]);
		freealign(ctlr->erst[i]);
		ctlr->erst[i] = nil;
	}
	free(ctlr->port), ctlr->port = nil;
	free(ctlr->slot), ctlr->slot = nil;
	freealign(ctlr->dcba), ctlr->dcba = nil;
	freealign(ctlr->sba), ctlr->sba = nil;
	if(ctlr->sbp != nil){
...
xhciinit(Hci *hp)
{
	...
	while((x = xecp(ctlr, 2, x)) != nil){
		...
		while(j--){
			...
			pp = &ctlr->port[i-1];
			pp->proto = x[0]>>16;
			PUT2(pp->spec, x[1]);		/* x is volatile MMIO; copy via a word read */
			PUT2(pp->spec+2, x[1]>>16);
	...
	for(i=0; i<ctlr->nintrs; i++){
		volatile u32int *irs = &ctlr->rts[IR0 + i*8];
	...
	print("usbxhci %llux: %d ports, %d slots, %d intrs (usbsts %#ux)\n",
		ctlr->base, hp->nports, ctlr->nslots, ctlr->nintrs, ctlr->opr[USBSTS]);

	kproc("xhcirecover", recover, hp, 0);
}
...
interrupt(Ureg*, void *arg)
{
	...
	for(x = ring->rp;; x=++ring->rp){
		...
		case ER_TRANSFER:
			...
			break;
		case ER_MFINDEXWRAP:
			ctlr->mframe = (ctlr->rts[MFINDEX] & (1<<14)-1) | 
				(ctlr->mframe+(1<<14) & ~((1<<14)-1));
			break;
		case ER_HCE:
...
freeslot(Slot *slot)
{
	...
	freering(&slot->epr[0]);
	freealign(slot->ibase);
	freealign(slot->obase);
	free(slot);
}
...
isoread(Ep *ep, uchar *p, long n)
{
	...
	for(i = io->frame;; i++){
		...
		d = io->b->rp + (i&io->ring->mask)*io->tdsz;
		dmaflush(1, d, m);
		queuetd(io->ring, TR_ISOCH | (i*mu/8 & 0x7ff)<<20 | TR_IOC, m,
			(*ctlr->dmaaddr)(d), nil);
	}
	io->frame = i;
